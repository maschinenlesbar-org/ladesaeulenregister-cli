...
test("performs a real GET and returns status, headers and body", async () => {
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({ method: "GET", url: `${baseUrl}/0/query?where=1%3D1` });
      assert.equal(resp.status, 200);
      assert.deepEqual(JSON.parse(resp.body.toString("utf8")), { path: "/0/query?where=1%3D1" });
    },
  );
});

test("rejects an unsupported protocol with LadesaeulenNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof LadesaeulenNetworkError && /timed out after 300ms/.test(err.message),
      );
